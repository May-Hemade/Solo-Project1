...
import { getProducts, getReviews, writeProducts, writeReviews } from "../../lib/fs-tools.js"
...
productRouter.post("/", checkProductSchema, triggerBadRequest, async (req, res, next) => {
  try {
...
productRouter.post("/:id/upload", parseFile.single("img"), uploadImage, async (req, res, next) => {
  try {
    const fileAsJSONArray = await getProducts()

    const index = fileAsJSONArray.findIndex((product) => product._id === req.params.id)
    if (index === -1) {
      next(createHttpError(404, `Product with id ${req.params.id} not found!`))
      return
    }
...
productRouter.get(
  ...
  async (req, res, next) => {
    try {
      const reviews = await getReviews()
      const reviewOfProduct = reviews.filter((review) => review.productId === req.params.id)
...
productRouter.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const oneReview = reviews.find((review) => review._id === req.params.reviewId)
      if (oneReview) {
        res.send(oneReview)
      } else {
...
productRouter.put(
  ...
  async (req, res, next) => {
    try {
      ...
      const index = reviews.findIndex((review) => review._id === req.params.reviewId)
...
productRouter.delete(
  ...
  async (req, res, next) => {
    try {
      const reviews = await getReviews()
      const remaining = reviews.filter((review) => review._id !== req.params.reviewId)
